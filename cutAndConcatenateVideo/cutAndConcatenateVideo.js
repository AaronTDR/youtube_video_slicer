...
async function concatenateSegments({
  ...
  try {
    ...
    const fileNameOutput = `${sortableDate}_final_result_${videoName}`;
...
async function cutAndConcatenateVideo(
  ...
  const fullPathVideo = `${workingFolderPath}${videoName}${fileExtension}`;
  ...
  try {
    ...
    const file = await extractSegments({
      ffmpeg_exe_path,
      fullPathVideo,
      timestampsInSeconds,
      keyframes,
    ...
    return await concatenateSegments({
      ffmpeg_exe_path,
      workingFolderPath,
      file,
      videoName,
